...
okexit *true|false*
: the `--okexit` option does the opposite of bash's `set -o errexit`: `xargs`
terminates if the command exits with status 0. If `--okexit=false`,
xargs will run the command on all nodes in the list.
